...
const STORAGE_TAB = "tasklog:tab:v1";
...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = useState<"today" | "pool" | "tomorrow">("today");
  ...
  useEffect(() => {
    try {
      ...
      if (raw === "today" || raw === "pool" || raw === "tomorrow") {
        setActiveTab(raw);
      }
    } catch {}
  }, []);
  ...
  function toggleMissed() {
    setSectionsCollapsed((prev) => ({ ...prev, missed: !prev.missed }));
  }
  ...
  useEffect(() => {
    ...
    if (lastSeen && lastSeen !== todayKey) {
      ...
      setItems((prev) =>
        prev.map((it) => {
          if (it.isTomorrow && !it.completedAt) {
  ...
  function doneToday(id: string) {
    const now = Date.now();
    // 完了時も isToday/isTomorrow/miniStep を保持したまま completedAt を付ける。
    // → やったことから「戻す」と今日/明日の元の場所に戻る（気になるに落ちない）。
    setItems((prev) =>
      prev.map((it) =>
        it.id === id
          ? { ...it, touches: [...it.touches, now], completedAt: now }
          : it
      )
    );
  ...
  const TABS: Array<{
  ...
  }> = [
    {
      key: "pool",
      label: "気になる",
      count: poolRoots.length,
      activeClass: "bg-neutral-700 text-neutral-50 border-neutral-500",
    },
    {
      key: "today",
      label: "今日",
      count: todayItems.length,
      activeClass: "bg-emerald-500/15 text-emerald-200 border-emerald-500/60",
    },
    {
      key: "tomorrow",
      label: "明日",
      count: tomorrowItems.length,
      activeClass: "bg-sky-500/15 text-sky-200 border-sky-500/60",
    },
  ];
  ...
  function renderTabButton(
    ...
    const base =
      variant === "top"
        ? "flex-1 rounded-md border px-3 py-1.5 text-sm font-semibold transition-colors flex items-center justify-center gap-1.5"
        : "flex-1 border-t-2 px-2 py-2 text-xs font-semibold transition-colors flex flex-col items-center gap-0.5";
    const inactiveTop =
    ...
    const cls =
      variant === "top"
        ? `${base} ${isActive ? tab.activeClass : inactiveTop}`
        : `${base} ${isActive ? activeBottom : inactiveBottom}`;
    return (
      <button
        key={tab.key}
  ...
  return (
    <main className="flex-1 bg-neutral-950 text-neutral-100 px-4 py-8 sm:px-8 pb-24 lg:pb-8">
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <section className="rounded-lg border border-neutral-800 bg-neutral-900 p-3 space-y-2">
          <input
            ...
            placeholder="気になってる事は？（Enter で「気になる」）"
            className="w-full rounded-md border border-neutral-700 bg-neutral-950 px-3 py-2 text-base sm:text-sm outline-none focus:border-emerald-500"
          />
        ...
        {/* デスクトップ：上部セグメント型タブ（中央寄せ） */}
        <div className="hidden lg:flex justify-center">
          <div className="flex gap-2 rounded-lg border border-neutral-800 bg-neutral-900 p-1 w-full max-w-md">
            {TABS.map((tab) => renderTabButton(tab, "top"))}
          </div>
        </div>

        {/* 今日タブ：今日やる事＋未達成（畳み）＋やったこと */}
        {activeTab === "today" && (
      ...
      {/* モバイル：画面下部に固定のボトムタブ（親指圏） */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 flex border-t border-neutral-800 bg-neutral-950/95 backdrop-blur pb-[env(safe-area-inset-bottom)]">
